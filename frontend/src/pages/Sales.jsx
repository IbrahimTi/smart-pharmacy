...
const Sales = () => {
  ...
  const handleDeleteSale = async (saleId) => {
    ...
    try {
      await fetch(
        `${import.meta.env.VITE_API_URL || "http://localhost:5000/api"}/sales/${saleId}`,
        {
          method: "DELETE",
          headers: {
      ...
      <html>
        <head>
          ...
          <style>
            ...
            .total-box { width: 300px; margin-left: auto; margin-top: 20px; font-size: 15px; }
            .total-row { display: flex; justify-content: space-between; padding: 7px 0; }
            .final { border-top: 2px solid #111827; font-weight: bold; font-size: 18px; }
        ...
        <body>
          ...
          <div class="info">
            <div>
              <p><strong>Invoice:</strong> ${safeText(selectedSale.invoiceNumber)}</p>
              <p><strong>Date:</strong> ${new Date(selectedSale.createdAt).toLocaleDateString()}</p>
              <p><strong>Payment:</strong> ${safeText(selectedSale.paymentMethod)}</p>
            </div>
            <div>
              <p><strong>Customer:</strong> ${safeText(selectedSale.customer?.name)}</p>
              <p><strong>Phone:</strong> ${safeText(selectedSale.customer?.phone)}</p>
              <p><strong>Pharmacist:</strong> ${safeText(selectedSale.pharmacist?.name)}</p>
            </div>
          </div>
          <table>
            <thead>
              <tr><th>SL</th><th>Medicine</th><th>Qty</th><th>Price</th><th>Subtotal</th></tr>
            </thead>
            <tbody>${itemRows}</tbody>
          </table>
          <div class="total-box">
            <div class="total-row"><span>Total</span><span>৳${selectedSale.totalAmount}</span></div>
            <div class="total-row"><span>Discount</span><span>৳${selectedSale.discount || 0}</span></div>
            <div class="total-row final"><span>Final Amount</span><span>৳${selectedSale.finalAmount}</span></div>
          </div>
          <div class="footer">
                ...
                {items.map((item, index) => (
                  <div key={index} style={{ display: "grid", gridTemplateColumns: "3fr 1fr 1fr auto", gap: "10px", marginBottom: "8px", alignItems: "center" }}>
                    ...
                    <input
                      ...
                      onBlur={e => { e.target.style.borderColor = "#e2e8f0"; e.target.style.background = "#f8fafc"; }}
                    />
                    <div style={{ background: "#f0fdf4", border: "1px solid #bbf7d0", borderRadius: "8px", padding: "9px 12px", fontSize: "0.875rem", fontWeight: 700, color: "#16a34a" }}>
                      ৳{item.subtotal}
                    </div>
                    {items.length > 1 && (
                      <button
                        ...
                        style={{ background: "#fef2f2", border: "1px solid #fecaca", color: "#dc2626", padding: "8px", borderRadius: "8px", cursor: "pointer", display: "flex", alignItems: "center" }}
                      >
                        <Trash2 size={14} />
                      </button>
                    )}
          ...
          <div style={{
            ...
            <div style={{
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px", borderBottom: "1px solid #e2e8f0", paddingBottom: "14px" }}>
                <div>
                  <h2 style={{ fontFamily: "'Sora', sans-serif", fontSize: "1.3rem", fontWeight: 800, color: "#0f172a" }}>
                    Invoice Details
                  </h2>
                ...
                <button
                  ...
                  style={{ background: "#fef2f2", border: "1px solid #fecaca", borderRadius: "8px", padding: "8px", cursor: "pointer", color: "#dc2626" }}
                >
                  <X size={20} />
                </button>
              </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px", marginBottom: "20px" }}>
                <div style={{ background: "#f8fafc", padding: "14px", borderRadius: "12px", border: "1px solid #e2e8f0" }}>
                  <p style={{ fontSize: "0.85rem", marginBottom: "6px", color: "#374151" }}><strong>Customer:</strong> {selectedSale.customer?.name}</p>
                  <p style={{ fontSize: "0.85rem", color: "#374151" }}><strong>Phone:</strong> {selectedSale.customer?.phone}</p>
                </div>
                <div style={{ background: "#f8fafc", padding: "14px", borderRadius: "12px", border: "1px solid #e2e8f0" }}>
                  <p style={{ fontSize: "0.85rem", marginBottom: "6px", color: "#374151" }}><strong>Date:</strong> {new Date(selectedSale.createdAt).toLocaleDateString()}</p>
                  <p style={{ fontSize: "0.85rem", marginBottom: "6px", color: "#374151" }}><strong>Pharmacist:</strong> {selectedSale.pharmacist?.name}</p>
                  <p style={{ fontSize: "0.85rem", color: "#374151" }}><strong>Payment:</strong> {selectedSale.paymentMethod}</p>
                </div>
              </div>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", marginBottom: "20px" }}>
                <thead>
                  <tr style={{ background: "#f1f5f9" }}>
                    {["SL", "Medicine", "Quantity", "Price", "Subtotal"].map(h => (
                      <th key={h} style={{ textAlign: "left", padding: "10px", borderBottom: "1px solid #e2e8f0", color: "#374151", fontSize: "0.75rem", textTransform: "uppercase" }}>
                        {h}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody>
                  {selectedSale.items?.map((item, index) => (
                    <tr key={index} style={{ borderBottom: "1px solid #f1f5f9" }}>
                      <td style={{ padding: "10px", color: "#374151" }}>{index + 1}</td>
                      <td style={{ padding: "10px", color: "#374151" }}>{item.medicineName || item.medicine?.name}</td>
                      <td style={{ padding: "10px", color: "#374151" }}>{item.quantity}</td>
                      <td style={{ padding: "10px", color: "#374151" }}>৳{item.price}</td>
                      <td style={{ padding: "10px", color: "#374151" }}>৳{item.subtotal}</td>
                    </tr>
                  ))}
                </tbody>
              </table>

              <div style={{ width: "280px", marginLeft: "auto", background: "#f8fafc", borderRadius: "12px", padding: "16px", border: "1px solid #e2e8f0" }}>
                <div style={{ display: "flex", justifyContent: "space-between", color: "#374151", fontSize: "0.9rem", marginBottom: "8px" }}>
                  <span>Total</span><strong>৳{selectedSale.totalAmount}</strong>
                </div>
                <div style={{ display: "flex", justifyContent: "space-between", color: "#374151", fontSize: "0.9rem", marginBottom: "8px" }}>
                  <span>Discount</span><strong>৳{selectedSale.discount || 0}</strong>
                </div>
                <div style={{ display: "flex", justifyContent: "space-between", borderTop: "2px solid #e2e8f0", paddingTop: "10px", color: "#059669", fontSize: "1.1rem" }}>
                  <span>Final</span><strong>৳{selectedSale.finalAmount}</strong>
                </div>
              </div>
              ...
              <div style={{ display: "flex", justifyContent: "flex-end", gap: "10px", marginTop: "22px" }}>
                <button
                  onClick={() => setShowInvoice(false)}
                  style={{ background: "#f1f5f9", color: "#64748b", border: "1px solid #e2e8f0", padding: "10px 18px", borderRadius: "8px", cursor: "pointer", fontWeight: 600 }}
                >
                  Close
                </button>
                <button
                  onClick={handlePrintInvoice}
                  style={{ display: "flex", alignItems: "center", gap: "8px", background: "#2563eb", color: "white", border: "none", padding: "10px 18px", borderRadius: "8px", cursor: "pointer", fontWeight: 700 }}
                >
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr style={{ background: "#f8fafc", borderBottom: "2px solid #e2e8f0" }}>
                {["Invoice", "Customer", "Items", "Amount", "Payment", "Pharmacist", "Date", "Actions"].map(h => (
                  <th key={h} style={{ textAlign: "left", padding: "13px 16px", fontSize: "0.72rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.05em", color: "#64748b" }}>
                    {h}
                  </th>
                ))}
              </tr>
                ...
                >
                  <td style={{ padding: "13px 16px" }}>
                    <button
                      ...
                      style={{ fontFamily: "monospace", fontSize: "0.8rem", fontWeight: 700, color: "#2563eb", background: "#eff6ff", padding: "5px 10px", borderRadius: "6px", border: "none", cursor: "pointer" }}
                    >
                      {sale.invoiceNumber}
                    </button>
                  </td>
                  ...
                  <td style={{ padding: "13px 16px" }}>
                    <span style={{ fontSize: "0.72rem", fontWeight: 600, padding: "3px 10px", borderRadius: "20px", background: paymentColors[sale.paymentMethod]?.bg || "#f1f5f9", color: paymentColors[sale.paymentMethod]?.color || "#64748b" }}>
                      {sale.paymentMethod}
                    </span>
                  </td>
                  ...
                  <td style={{ padding: "13px 16px" }}>
                    {user?.role === "admin" &&  (
                      <button
                        ...
                        style={{ background: "#fef2f2", border: "1px solid #fecaca", color: "#dc2626", padding: "6px", borderRadius: "8px", cursor: "pointer", display: "flex", alignItems: "center", transition: "all 0.2s" }}
